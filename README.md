...
To add a custom email template for `/register` and `/forgotPassword` APIs, define it in the request body data. Since we cannot send HTML data directly to the request using JSON, convert it into a string using the `/htmlToString API`.
...
To add a custom validation message for any field, add a messages property to the request data. For details of validation keys, check the [Additional Details](#additional-details) section.
```json
{
    "messages": {
        "email": {
