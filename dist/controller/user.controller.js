...
const userMeta_model_1 = require("../models/userMeta.model");
...
const getListOfUser = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        let getUsersData = yield user_model_1.User.findAll({});
...
const registerUser = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        let user = yield user_model_1.User.findOne({ where: { email: reqData.email } });
        if (user && user.status == user_model_1.status.DEACTIVE) {
            yield user_model_1.User.update(jsonObj, { where: { email: reqData.email } });
            ...
            let createUser = yield user_model_1.User.create(jsonObj);
            if (createUser) {
                ...
                yield userMeta_model_1.UserMeta.create(metaObj);
                ...
                if (config_json_1.default.CUSTOM_TEMPLATE == true) {
                ...
                const otpHtmlMessage = yield (0, utils_1.generateOtpHtmlMessage)(jsonObj.email, config_json_1.default.CUSTOM_TEMPLATE, customOtpHtmlTemplate, "Registration done successfully.Here is your OTP for varification.", templatedata);
...
const verifyOTP = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const user = yield user_model_1.User.findOne({ where: { email }, raw: true });
        if (!user) {
        ...
        const userMeta = yield userMeta_model_1.UserMeta.findOne({ where: { userId: user.id, key: constant_1.keyName } });
        ...
        yield user_model_1.User.update({ status: user_model_1.status.ACTIVE, isVerified: true }, { where: { email } });
        yield userMeta_model_1.UserMeta.update({ value: null }, { where: { userId: user.id, key: constant_1.keyName } });
...
const resendOTP = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        yield userMeta_model_1.UserMeta.update({ value: newOTP.toString() }, { where: { userId: user.id, key: constant_1.keyName } });
        ...
        const otpHtmlMessage = yield (0, utils_1.generateOtpHtmlMessage)(user.email, config_json_1.default.CUSTOM_TEMPLATE, customTemplate, "OTP Verification.", templatedata);
        return res.status(200).json({
            status: true,
...
const forgotPassword = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        yield userMeta_model_1.UserMeta.update({ value: newOTP.toString() }, {
            where: { userId: user.id, key: constant_1.keyName },
        });
        // Send the new OTP to the user's email
        const otpHtmlMessage = `<p>Your new OTP code for update password: <b>${newOTP}</b></p>`;
        (0, email_helper_1.sendEmail)(email, "Forgot Password OTP", otpHtmlMessage);
        return res.status(200).json({
            status: true,
            message: res.__("SENT_OTP"),
        });
...
const resetPassword = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        yield Promise.all([
            user_model_1.User.update({ password: hashedPassword }, { where: { id: user.id } }),
            userMeta_model_1.UserMeta.update({ value: null }, { where: { userId: user.id, key: constant_1.keyName } }),
        ]);
        return res.status(200).json({
            status: true,
...
const logIn = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const token = yield (0, auth_helper_1.generateToken)({ id: user.id, email: user.email });
...
const changePassword = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        yield user_model_1.User.update({ password: hashedNewPassword }, { where: { id: userId } });
        return res.status(200).json({
            status: true,
...
const checkValidation = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const userAttributes = Object.keys(user_model_1.User.getAttributes());
        ...
        const existingUser = yield user_model_1.User.findOne({ where: query });
...
const deleteUser = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        if (!config_json_1.default.HARD_DELETE) {
            const modelAttributes = Object.keys(user_model_1.User.getAttributes());
            if (!modelAttributes.includes('isDeleted')) {
                return res.status(500).json({ status: false, message: res.__("FIELD_NOT_FOUND") });
            }
            const softDeleteResult = yield user_model_1.User.update({ isDeleted: true }, { where: { id: userId, isDeleted: { [sequelize_1.Op.or]: [false, null] } } });
            ...
            const result = yield user_model_1.User.destroy({ where: { id: userId } });
            if (result === 1) {
                return res.status(200).json({ status: true, message: res.__("USER_DELETED") });
            }
            else {
...
const profileUpload = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const [updatedRows] = yield user_model_1.User.update({ profileImage: profileImage.filename }, { where: { id: userId } });
        if (updatedRows === 0) {
            return res.status(404).json({ status: false, message: res.__('USER_NOT_FOUND') });
        }
        return res.status(200).json({ status: true, message: res.__('IMAGE_UPLOADED'), data: config_json_1.default.BASE_URL + `/${profileImage.destination}` + profileImage.filename });
    }
    catch (e) {
        console.error(e);
        return res.status(500).json({
...
const updateConfig = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const result = (0, email_helper_1.updateEmailConfig)(newConfig);
